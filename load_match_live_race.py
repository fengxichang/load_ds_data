...
class Handler(BaseHandler):
    ...
    def index_page(self, response):
        ...
        match_list_res = requests.get("http://local.ds.football/api/match/getMatchByDate?startDate="+start+"&endDate="+endDate)

        match_list = json.loads(match_list_res.content)

        for match_id in match_list:
            self.crawl("https://www.dszuqiu.com/race_sp/"+match_id, callback=self.detail_page, validate_cert=False, headers=get_headers(), cookies=random_bid())
    ...
    def detail_page(self, response):
        ...
        for tr in response.doc("div[class='small-4 columns tableCol3in1 daxiao']").find("table[class='responsive half'] > tbody").items("tr"):
            ...
            time = tr.children("td").eq(0).text()[:-1]
            score = tr.children("td").eq(1).text()
            big = tr.children("td").eq(2).text()
            small = tr.children("td").eq(3).text()
            bsp = tr.children("td").eq(4).text()
            _type = 0
